...
if args.display in ('ssd1306', 'ssd1331', 'sh1106'):
    ...
    Device = getattr(device, args.display)
    if args.interface == 'i2c':
        serial = serial.i2c(port = args.i2c_port, address = args.i2c_address)
    elif args.interface == 'spi':
        serial = serial.spi(port = args.spi_port,
    ...
    Emulator = getattr(emulator, args.display)
